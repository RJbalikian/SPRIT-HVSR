...
def run(input_data=None, source='file', azimuth_calculation=False, noise_removal=False, outlier_curves_removal=False, suppress_report_outputs=False, verbose=False, **kwargs):
    ...
    **kwargs
    ...
    # Show print report by default unless otherwise specified
    if 'show_print_report' not in get_report_kwargs.keys():
        get_report_kwargs['show_print_report'] = True
    ...
    hvsr_results = get_report(hvsr_results=hvsr_results, 
                              suppress_report_outputs=suppress_report_outputs,
                              verbose=verbose, **get_report_kwargs)
    ...
    **readcsv_getMeta_fetch_kwargs
    ...
    if batch_type == 'table':
        ...
        if sample_data:
            ...
            if str(batch_data)=='sample':
                dataReadInfoDF = pd.read_csv(SAMPLE_DATA_DIR.joinpath('Batch_SampleData.csv'), **read_csv_kwargs)
                sampleFileDataList = []
                for i,site_row in dataReadInfoDF.iterrows():
                    sampleFileDataList.append(SAMPLE_DATA_DIR.joinpath(site_row['input_data']))
                dataReadInfoDF['input_data'] = sampleFileDataList
...
def export_report(hvsr_results, report_export_path=None, report_export_format=['pdf'], azimuth='HV', csv_handling='rename', show_report=True, verbose=False):
    ...
    for ref in report_export_format:
        if report_export_path is None:
            ...
            print(f'The {ref} report will be saved in the home directory: {pathlib.Path.home()}')

        tableList = ['table', 'dataframe', 'spreadsheet', 'excel', 'csv']
        plotList = ['plot', 'chart', 'graph', 'image', 'png']
        printList = ['print', 'text', 'printed', 'txt']
        htmlList = ['webpage', 'web', 'html']

        if str(ref).lower() in tableList:
            ref = 'table'
            ext = '.csv'
        elif str(ref).lower() in plotList:
            ref = 'plot'
            ext = '.png'
        elif str(ref).lower() in printList:
            ref = 'print'
            ext = '.txt'
        elif str(ref).lower() in htmlList:
            ref = 'html'
            ext = '.html'
        else:
            ref == 'pdf'
        ...
        fname = f"{sitename}_{str(ref).upper()}REPORT_{hvsr_results['hvsr_id']}_{datetime.date.today()}{ext}"
        fname = fname.replace(':', '')
    ...
    **kwargs
    ...
    try:
        ...
        if 'obspy_ppsds' in kwargs and kwargs['obspy_ppsds']:
            ...
            if verbose:
                print("\n\tMetadata/instrument response does not match data.")
...
def generate_psds(hvsr_data, window_length=30.0, overlap_pct=0.5, window_type='hann', window_length_method='length', 
    ...
    # if (verbose and isinstance(hvsr_data, HVSRBatch)) or (verbose and not hvsr_data['batch']):
    if verbose:
        print('\nGenerating Power Spectral Densities (generate_psds())')
        print('\tUsing the following parameters:')
        for key, value in orig_args.items():
            if key == 'hvsr_data':
                pass
...
def get_report(hvsr_results, report_formats=['print', 'table', 'plot', 'html', 'pdf'], azimuth='HV',
               ...
               show_print_report=False, show_table_report=False, show_plot_report=False, show_html_report=False, show_pdf_report=False,
    ...
    * 'pdf': A PDF document showing the summary of the HVSR Results. 
    ...
    if isinstance(report_export_format, (list, tuple)):
        report_export_format = [str(ref).lower() for ref in report_export_format]
        ...
        allList = [':', 'all']
        if str(report_export_format).lower() in allList:
            report_export_format = ['print', 'table', 'plot', 'html', 'pdf']
        else:
            report_export_format = [report_export_format]

    # Put print first to get results immediately while plots or others are created
    ...
    for ref in report_export_format:
        if ref not in report_formats:
            report_formats.append(ref)

    # Loop through all the report formats to be generated (also will check if report has been specified for export)
    for i, rep_form in enumerate(report_formats):
        ...
        if str(rep_form).lower() == 'print':
            ...
            if 'print' in report_export_format:
                if pathlib.Path(exp_path).is_dir():
                    print_exp_path = pathlib.Path(exp_path).joinpath(f"{hvsr_results.site}_TextReport_{hvsr_results.hvsr_id}.txt")
                else:
        ...
        # Table_Report
            ...
            if 'table' in report_export_format:
                if exp_path is None:
                    ...
                    if pathlib.Path(exp_path).is_dir():
                        table_exp_path = pathlib.Path(exp_path).joinpath(f"{hvsr_results.site}_TableReport_{hvsr_results.hvsr_id}.csv")
                    else:
        ...
        # Plot_Report
            ...
            if 'plot' in report_export_format:
                export_report(hvsr_results=hvsr_results, report_export_path=exp_path, report_export_format='plot')
        ...
        # HTML_Report
            ...
            if 'html' in report_export_format:
                if pathlib.Path(exp_path).is_dir():
                    html_exp_path = pathlib.Path(exp_path).joinpath(f"{hvsr_results.site}_HTMLReport_{hvsr_results.hvsr_id}.html")
                else:
...
def import_data(import_filepath, data_format='gzip', show_data=True):
    ...
    sample_list = ['sample', 'sample1', 'sample01', 'sample2', 'sample02', 'sampledata', 's', '1', '2']
    if str(import_filepath).lower() in sample_list:
        if '2' in str(import_filepath):
            import_filepath = SAMPLE_DATA_DIR.joinpath(r'SampleHVSRSite02.hvsr')
        else:
            import_filepath = SAMPLE_DATA_DIR.joinpath(r'SampleHVSRSite01.hvsr')
...
def remove_noise(hvsr_data, remove_method=None, 
                 ...
                 std_ratio_thresh=1.5, std_window_size=20.0, min_std_win=5.0,
...
def _generate_print_report(hvsr_results, azimuth="HV", show_print_report=True, verbose=False):
    ...
    minutes = int((hvsr_results.endtime - hvsr_results.starttime)/60)
    sec = (((hvsr_results.endtime - hvsr_results.starttime)/60) - minutes) * 100
    julDay = obspy.UTCDateTime(hvsr_results.acq_date).julday
    durationStr = f"{minutes}m {sec:.6f}s"
    acqTimeInfo = f"{hvsr_results['input_params']['acq_date']} {hvsr_results.starttime.strftime("%T")}-{hvsr_results.endtime.strftime("%T")} (Day of Year: {julDay})"
    ...
    report_string_list.append(f"\tAcq. Time: {acqTimeInfo}")
    report_string_list.append(f"\tDuration : {durationStr}")
...
def _generate_pdf_report(hvsr_results, pdf_report_filepath=None, show_pdf_report=False, show_html_report=False, return_pdf_path=False, verbose=False):
    ...
    if pdf_report_filepath is None:
        if verbose:
            print('\t The pdf_report_filepath parameter was not specified, attempting to save to temporary file.')
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as temp_file:
                pdf_export_path = temp_file.name  # Get the name of the temporary file
            pdf_export_path = pathlib.Path(pdf_export_path).parent.joinpath(f'tmp_{hvsr_results.site[:16]}_Report_{hvsr_results.hvsr_id}_{pathlib.Path(temp_file.name).stem}.pdf').as_posix()
            ...
            print(f'\t\t PDF Report saved to {temp_file.name}')
        except Exception:
            try:
                print(f"\t Temporary file could not be saved to {pdf_export_path}. \n\t\tAttempting to write file to working directory: {pathlib.Path().cwd()}")
                pdf_export_path = pathlib.Path().cwd().as_posix()
                with open(pdf_export_path, 'wb') as cwd_file:
                    pisa_status = pisa.CreatePDF(htmlReport, dest=cwd_file)
            except:
                print("\t File could not be saved in working directory, attempting creation of PDF data using BytesIO")
                output = io.BytesIO()
                ...
                print('\t\t PDF File saved as bytes buffer in the PDF_Report attribute of HVSRData object')
